use lox_bytecode::{
	bytecode::{Class, Closure, Function},
	opcode,
};
use lox_syntax::{
	ast::{
		Ast, BinaryOperator, BorrowedAst, Expr, Identifier, LogicalOperator, Stmt, UnaryOperator,
	},
	position::WithSpan,
};

use super::compiler::{Compiler, ContextType};

pub fn compile_ast(compiler: &mut Compiler, ast: BorrowedAst<'_>) {
	for stmt in ast {
	...
	match &stmt.value {
		...
		Stmt::Return(ref expr) => {
			if compiler.context_type() == ContextType::Initializer && expr.is_some() {
				...
				compiler.add_error("Invalid return".to_owned(), stmt.span);
				return;
			}
...
fn declare_variable<I: AsRef<str>>(compiler: &mut Compiler, identifier: WithSpan<I>) {
	if compiler.is_scoped() {
		if compiler.has_local_in_current_scope(identifier.value.as_ref()) {
			compiler.add_error("Local already defined".to_owned(), identifier.span);
			return;
		}

		compiler.add_local(identifier.value.as_ref().to_owned());
	}
}

fn define_variable(compiler: &mut Compiler, identifier: String) {
	if compiler.is_scoped() {
...
fn compile_import(
	...
	let constant = compiler.add_string(path.value.clone());
	...
	if let Some(identifiers) = identifiers {
		for identifier in identifiers {
			...
			let constant = compiler.add_identifier(identifier.value.clone());
			compiler.add_u8(opcode::IMPORT_GLOBAL);
			compiler.add_u32(constant as _);

			define_variable(compiler, identifier.value.clone());
		}
	}
...
fn compile_closure(
	...
	let (chunk_index, upvalues) = compiler.with_scoped_context(context_type, |compiler| {
		for arg in args {
			...
			define_variable(compiler, arg.value.clone());
		}
...
fn compile_var_declaration<T: AsRef<WithSpan<Expr>>, I: AsRef<str>>(
	...
	define_variable(compiler, identifier.value.as_ref().to_owned());
}
...
#[allow(clippy::disallowed_macros, clippy::match_wildcard_for_single_variants)]
fn compile_expr(compiler: &mut Compiler, expr: &WithSpan<Expr>) {
	match expr.value {
		Expr::Number(num) => compile_number(compiler, num),
		Expr::String(ref string) => compile_string(compiler, string.clone()),
		...
		Expr::Unary(ref operator, ref expr) => compile_unary(compiler, *operator, expr),
...
fn compile_list(compiler: &mut Compiler, expr: &[WithSpan<Expr>]) {
	for expr in expr {
		compile_expr(compiler, expr);
	}
...
fn compile_this(compiler: &mut Compiler, expr: &WithSpan<Expr>) {
	if !compiler.in_method_or_initializer_nested() {
		compiler.add_error("Invalid 'this'".to_owned(), expr.span);
		return;
	}

	compile_variable(compiler, WithSpan::new(&"this".to_owned(), expr.span));
}
...
fn compile_call(compiler: &mut Compiler, identifier: &WithSpan<Expr>, args: &[WithSpan<Expr>]) {
	if let Expr::Get(expr, ident) = &identifier.value {
		...
		let constant = compiler.add_identifier(ident.value.clone());
...
fn compile_string(compiler: &mut Compiler, string: String) {
