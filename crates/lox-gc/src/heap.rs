...
impl AddrSpace {
	...
	pub(crate) fn new_reserved(&self) -> PdList {
		debug_assert_eq!(
			...
			self.used_pds.get(),
			"You cannot call new_reverved after init"
		);
	...
	pub fn new_ream(&self, count: u32) -> PdRef<'_> {
		let used_pages = self.used_pds.get();
	...
	pub unsafe fn sweep(&self) {
		...
		for pd in self.space.pds() {
			...
			if pd.is_empty() {
				...
				debug_assert!(!matches!(pd.class(), SizeClass::Ream));
				pd.unlink();
...
mod tests {
	...
	fn it_works() {
		...
		let _ptr2 = heap.alloc(layout).cast::<u64>();
		assert_eq!(_ptr, _ptr2);
	}
}
