use super::{
	...
	common::{expect_identifier, expect_string},
	parser::Parser,
...
fn parse_statement(it: &mut Parser<'_>) -> Option<StmtWithSpan> {
	match it.peek() {
...
fn parse_class_declaration(it: &mut Parser<'_>) -> Option<StmtWithSpan> {
	...
	Some(WithSpan::new(
		Stmt::Class(name, superclass, functions),
		Span::union(begin_span, end_span),
	))
}

fn parse_function_declaration(it: &mut Parser<'_>) -> Option<StmtWithSpan> {
	let begin_span = it.expect(TokenKind::Fun)?;
	let fun = parse_function(it)?;

	let span = Span::union(begin_span, &fun);
	Some(WithSpan::new(fun.value, span))
}

fn parse_function(it: &mut Parser<'_>) -> Option<StmtWithSpan> {
	let name = expect_identifier(it)?;
...
fn parse_params(it: &mut Parser<'_>) -> Option<Vec<WithSpan<Identifier>>> {
...
fn parse_var_declaration(it: &mut Parser<'_>) -> Option<StmtWithSpan> {
	let begin_span = it.expect(TokenKind::Var)?;
	let name = expect_identifier(it)?;
	let initializer = if it.optionally(TokenKind::Equal)? {
		Some(parse_expr(it)?)
	} else {
		None
	};
...
fn parse_expr(it: &mut Parser<'_>) -> Option<WithSpan<Expr>> {
	super::expr_parser::parse(it)
}

fn parse_for_statement(it: &mut Parser<'_>) -> Option<StmtWithSpan> {
...
fn parse_import_statement(it: &mut Parser<'_>) -> Option<StmtWithSpan> {
...
fn parse_return_statement(it: &mut Parser<'_>) -> Option<StmtWithSpan> {
...
fn parse_expr_statement(it: &mut Parser<'_>) -> Option<StmtWithSpan> {
	let expr = parse_expr(it)?;
	let end_span = it.expect(TokenKind::Semicolon)?;

	let span = Span::union(&expr, end_span);
	Some(WithSpan::new(Stmt::Expression(Box::new(expr)), span))
}

fn parse_block_statement(it: &mut Parser<'_>) -> Option<StmtWithSpan> {
...
fn parse_while_statement(it: &mut Parser<'_>) -> Option<StmtWithSpan> {
...
fn parse_if_statement(it: &mut Parser<'_>) -> Option<StmtWithSpan> {
	...
	let else_stmt = if it.optionally(TokenKind::Else)? {
...
fn parse_print_statement(it: &mut Parser<'_>) -> Option<StmtWithSpan> {
...
pub fn parse(it: &mut Parser<'_>) -> Option<Vec<StmtWithSpan>> {
	parse_program(it)
}
...
mod tests {
	...
	use super::*;
	use crate::{position::Diagnostic, tokenizer::tokenize_with_context};

	fn parse_str(data: &str) -> Result<Vec<StmtWithSpan>, Vec<Diagnostic>> {
		let tokens = tokenize_with_context(data);
		let mut parser = Parser::new(&tokens);
		parse(&mut parser).map_or_else(|| Err(parser.diagnostics().to_owned()), Ok)
	}

	const fn ws<T>(value: T, range: Range<u32>) -> WithSpan<T> {
	...
	fn make_span_string(string: String, offset: u32) -> WithSpan<String> {
		let len = string.len() as u32;
		unsafe { WithSpan::new_unchecked(string, offset, offset + len) }
	}
	...
	fn var_decl() {
		assert_eq!(
			...
			Ok(vec![ws(
				Stmt::Var(make_span_string("beverage".to_owned(), 4), None),
				0..13
			),])
		);
		assert_eq!(
			...
			Ok(vec![ws(
				Stmt::Var(
					make_span_string("beverage".to_owned(), 4),
		...
		assert_errors("if (nil) var beverage = nil;", &["Unexpected 'var'"]);
	}
	...
	fn class_inheritance() {
		...
		assert_errors("class BostonCream < {}", &["Expected identifier got '{'"]);
		assert_errors(
	...
	#[test]
	fn r#for() {
		const fn block(what: Vec<WithSpan<Stmt>>, r: Range<u32>) -> WithSpan<Stmt> {
