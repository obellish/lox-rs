use std::{alloc::Layout, cell::Cell, mem, ops::Add};
...
impl Heap {
	pub fn new() -> Option<Self> {
		...
		let heap = Self {
			free_reams: space.create_reserved(),
			free_pages: space.create_reserved(),
			free_sized: [
				space.create_reserved(),
				space.create_reserved(),
				space.create_reserved(),
				space.create_reserved(),
				space.create_reserved(),
			...
			full_pages: space.create_reserved(),
			full_reams: space.create_reserved(),
			space,
			bytes_used: Cell::new(0),
		};

		let ream = heap.space.create_ream(heap.space.available_pages());
	...
	pub fn alloc(&self, layout: Layout) -> *mut u8 {
		...
		if bytes <= AddrSpace::PAGE_BYTES {
			...
			let class = SizeClass::SMALL[bin_log.saturating_sub(4) as usize];
	...
	pub unsafe fn start_gc(&self) {
		self.space.pds().for_each(PdRef::set_empty);
	}

	pub unsafe fn mark(&self, ptr: *const u8) {
		let page = unsafe { self.space.pd_of(ptr) };
		let offset = unsafe { ptr.offset_from(page.data(0)) } as usize;
	...
	pub unsafe fn sweep(&self) {
		...
		for pd in self.space.pds() {
			...
			if pd.is_empty() {
				...
				debug_assert_ne!(pd.class(), SizeClass::Ream);
				pd.unlink();
	...
	const fn sized_list(&self, size_class: SizeClass) -> &PdList {
		&self.free_sized[size_class as usize - 4]
	}

	fn take_ream(&self, pages: u32) -> PdRef<'_> {
		#[inline(never)]
		#[cold]
		...
		for ream in self.free_reams.iter(&self.space) {
			let Some((ream, rest)) = ream.split(pages as _) else {
				continue;
			};

			match rest {
				Some(r) if r.is_single_page() => self.free_pages.push(r),
				Some(r) => self.free_reams.push(r),
				None => {}
			}
	...
	fn alloc_small(&self, size_class: SizeClass) -> *mut u8 {
		...
		let page = list.first(&self.space).map_or_else(
			|| {
				let page = self.free_pages.first(&self.space).map_or_else(
					|| self.take_ream(1),
					|page| {
						page.unlink();
						page
...
impl AddrSpace {
	...
	const BITMAP_PAGES: usize = bytes_to_pages(Self::DATA_PAGES * mem::size_of::<Bitmap>());
	const DATA_BYTES: usize = 4 * 1024 * 1024 * 1024;
	...
	const PD_PAGES: usize = bytes_to_pages(Self::DATA_PAGES * mem::size_of::<PageDescriptor>());
	...
	pub fn new() -> Self {
		let mem = MemoryMap::new(Self::TOTAL_BYTES);
	...
	pub const unsafe fn pd(&self, idx: PdIdx) -> PdRef<'_> {
		PdRef { idx, space: self }
	}

	pub const unsafe fn pd_at(&self, index: u32) -> PdRef<'_> {
		PdRef {
			idx: PdIdx(index),
			space: self,
		}
	...
	pub const unsafe fn pd_of(&self, ptr: *const u8) -> PdRef<'_> {
		unsafe {
			let base = self.mem.data().add(Self::DATA_START);
			let offset = ptr.offset_from(base.cast_const()) as usize;
			let index = offset / Self::PAGE_BYTES;
			self.pd_at(index as _)
		}
	}

	pub fn pds(&self) -> impl Iterator<Item = PdRef<'_>> {
	...
	pub fn create_reserved(&self) -> PdList {
		debug_assert_eq!(
			self.reserved_pds.get(),
			self.used_pds.get(),
			"new_reserved cannot be called after init"
		);

		let ream = self.create_ream(1);
		self.reserved_pds.set(self.reserved_pds.get() + 1);

		PdList::new(ream.idx)
	}

	pub fn create_ream(&self, count: u32) -> PdRef<'_> {
		let used_pages = self.used_pds.get();
...
impl<'space> PdRef<'space> {
	fn pd(self) -> &'space PageDescriptor {
		unsafe {
			let offset = self.idx.0 as usize * mem::size_of::<PageDescriptor>();
	...
	fn bitmap(self) -> &'space Bitmap {
		unsafe {
			let offset = self.idx.0 as usize * mem::size_of::<Bitmap>() + AddrSpace::PD_BYTES;
	...
	pub fn set_empty(self) {
		for word in &self.bitmap().0 {
			word.set(0);
		}
	}

	pub fn is_empty(self) -> bool {
		self.bitmap().0.iter().all(|w| matches!(w.get(), 0))
	}
	...
	fn bytes_used(self) -> u32 {
		...
		if matches!(class, SizeClass::Ream) {
			if self.is_empty() {
				0
			} else {
...
impl PdList {
	pub const fn new(root: PdIdx) -> Self {
		Self { root }
	}
...
const fn bytes_to_pages(n: usize) -> usize {
	if matches!(n % AddrSpace::PAGE_BYTES, 0) {
...
mod tests {
	...
	fn it_works() {
		...
		let _ptr = heap.alloc(layout).cast::<u64>();
		let ptr = heap.alloc(layout).cast::<u64>();
		assert!(heap.is_marked(ptr as *const u8));
		unsafe {
			ptr.write(1234);
		...
		assert!(heap.is_marked(ptr as *const u8));
		assert!(!heap.is_marked(_ptr as *const u8));

		let ptr2 = heap.alloc(layout).cast::<u64>();
